...
const pulsingDot = (map: maplibregl.Map, color?: string) => ({
  width: size,
  height: size,
  ...
  render() {
    ...
    context!.fillStyle = color || gradient || "rgb(139, 92, 246)";
    context!.strokeStyle = color || gradient || "rgb(139, 92, 246)";
...
export const map = (
  ...
  map.on("load", () => {
    data.forEach((country, index) => {
      map.addImage(
        `pulsing-dot-${index}`,
        pulsingDot(map, getChartColor(index)),
        { pixelRatio: 2 }
      );
      ...
      map.on("mousemove", `points-${index}`, (e) => {
        if (e.features?.[0].geometry.type === "Point") {
          ...
          if (currentFeatureCoordinates !== featureCoordinates) {
            ...
            popup
              ...
              .setHTML(
                `
              <div style="background-color: ${getChartColor(index)}; padding: 12px; border-radius: 8px;">
                <p>${properties?.name}</p>
                <div class="data">
                  <div>Percentage</div>
      ...
      map.on("mouseleave", `points-${index}`, () => {
